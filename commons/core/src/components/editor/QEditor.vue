...
const initEditor = async (options: QuillOptions): Promise<void> => {
    ...
    editor.value = new Quill(editorRef.value, getQuillEditorOptions(merge(defaultOptions, options)));
};
...
watch(content, (value: string): void => {
    if (!isNull(editor.value) && !isEqual(editor.value.getSemanticHTML(), value)) {
