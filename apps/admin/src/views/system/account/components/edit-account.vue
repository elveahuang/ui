<template>
    <el-dialog v-model:open="open" class="app-modal" :title="$t('common.account_pages_edit_title')" @ok="handleSubmit" @cancel="handleCancel">
        <el-card>
            <el-form ref="entityForm" :model="model" :rules="rules" :label-col="{ span: 4 }" :wrapper-col="{ span: 12 }">
                <el-form-item name="username" :label="$t('common.field_account_name')">
                    <label>{{ model.username }}</label>
                </el-form-item>
                <el-form-item name="email" :label="$t('common.field_account_name')">
...
import { accountDetails, accountSaveApi } from '@commons/core/api/admin/account.ts';
import { AccountEntity } from '@commons/core/types/account.ts';
import { SEX_TYPES } from '@commons/core/types/user.type.ts';
...
watch(id, async (value) => {
    if (Number(value) > 0) {
        await accountDetails({ id: value }).then((result) => {
